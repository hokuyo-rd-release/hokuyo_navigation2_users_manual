...
#policy[
  ...
  自律走行で使うオドメトリは、#tsuyo[RSF の LIO（LiDAR 慣性オドメトリ）だけ]です。
  車輪の回転数からロボットの移動量を求める処理は、どこにも入っていません。

  #tsuyo[車輪オドメトリも含めて使いたい場合は、本ソフトウェアの対象外です。]
  TF の管理方法と、車輪オドメトリと LIO を融合する仕組みは、
  #tsuyo[お客様側でご用意ください。]
  モータドライバを差し替えるときの注意点は @subsubsec-setup-userdriver-tf にまとめています。
]

#plain[
  「オドメトリ」とは、#tsuyo[ロボットが自分でどれだけ動いたかを見積もった値]のことです。
  車輪の回転数から求める方法（車輪オドメトリ）が一般的ですが、
  本ソフトウェアは #tsuyo[LiDAR で見た景色の動きと IMU の傾き]から求めています。
  車輪が滑る路面でもずれにくいのが利点です。
]
